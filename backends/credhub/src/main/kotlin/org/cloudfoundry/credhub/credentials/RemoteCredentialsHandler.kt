...
class RemoteCredentialsHandler(
    ...
    override fun deleteCredential(credentialName: String) {
        ...
        try {
            ...
            if (e.status == io.grpc.Status.NOT_FOUND) {
                throw org.cloudfoundry.credhub.exceptions.EntryNotFoundException(org.cloudfoundry.credhub.ErrorMessages.Credential.INVALID_ACCESS)
            }
            throw RuntimeException("Request failed with status code: ${e.status.code}")
        }
    }
